...
def main():
    ...
    if nhbool:
        ...
        for item in a:
            ...
            newlink = 'http://www.hbrowse.com' + href
            driver.get(newlink)
            span = driver.find_elements_by_xpath('//span[@id="jsPageList"]/a')
            pages = span[-1].text
            pages = int(pages)
            if pages <= 1:
                time.sleep(2)
                src = driver.find_element_by_xpath("//td[@class='pageImage']/a[@name='next']/img[@id='mangaImage']").get_attribute('src')
            else:
                desired_tab = driver.current_window_handle
                try:
                    driver.find_element_by_xpath("//td[@class='pageImage']/a").click()
                except StaleElementReferenceException:
                    driver.find_element_by_xpath("//td[@class='pageImage']/a").click()
                if driver.current_window_handle != desired_tab:
                    driver.switch_to_window(desired_tab)
                time.sleep(2)
                src = driver.find_element_by_xpath("//td[@class='pageImage']/a[@name='next']/img[@id='mangaImage']").get_attribute('src')
            if src.find('lockedManga') != -1:
                console.print("Locked manga detected that couldn't be resolved in %s. Continuing if another chapter exists..." % chptitle, style = 'bold red')
                continue
            else:
                console.print("Locked manga problem in %s resolved!" % chptitle, style= 'bold green')
            driver.back()
            time.sleep(2)
            src = driver.find_element_by_xpath("//td[@class='pageImage']/a[@name='next']/img[@id='mangaImage']").get_attribute('src')
            typeof = src[-4:]
            ...
            start = time.time()
            for i in range(1, pages + 1):
                src = driver.find_element_by_xpath("//td[@class='pageImage']/a/img[@id='mangaImage']").get_attribute('src')
                try:
                    driver.find_element_by_xpath("//td[@class='pageImage']/a").click()
                except StaleElementReferenceException:
                    driver.find_element_by_xpath("//td[@class='pageImage']/a").click()
